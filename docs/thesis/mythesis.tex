...
%Source Model Creation and Refactoring Detection It supports the detection of 15 refactoring types for 4 different kinds of code elements, as shown in Table 1. This is a representative set of refactoring types, because it covers all structural code elements (i.e., packages, types, methods, and fields), and also covers control-flows
%of program statements (e.g., Extract and Inline Method).
...
\KwOut{Set MS of matched source models, Sets of unmatched nodes $U_{S1}$, $U_{S2}$ from $S_{1}$ and $S_{2}$, respectively}
