...
import io.jsrminer.util.AstUtils;
...
public enum RefactoringType {
    ...
    MOVE_RENAME_FILE("Move And Rename File", "Move And Rename File (.+) from (.+) to (.+)"),
    ...
    MOVE_RENAME_OPERATION("Move And Rename Method", "Move And Rename Method (.+) from  (.+) to (.+) from (.+)"),
    ...
    EXTRACT_MOVE_OPERATION("Extract And Move Method", "Extract And Move Method (.+) extracted from (.+) in class (.+) & moved to class (.+)"),
    MOVE_INLINE_OPERATION("Move And Inline Method", "Move And Inline Method (.+) moved from class (.+) to class (.+) & inlined to (.+)"),
    ...
    CHANGE_VARIABLE_KIND("Change Variable Kind", "Change Variable Type (.+) to (.+) in method (.+) from class (.+)"),
