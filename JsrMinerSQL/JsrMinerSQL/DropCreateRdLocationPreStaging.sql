...
DROP TABLE IF EXISTS dbo.RdPreStagingLocation;
...
FROM dbo.RdPreStaging

SELECT id, file_before, file_after, file_location_before, file_location_after
, CASE WHEN  CHARINDEX( '|', file_location_before) > 0 THEN 1 ELSE NULL END AS has_line_before
, CASE WHEN  CHARINDEX( '|', file_location_after) > 0 THEN 1 ELSE NULL END AS has_line_after
INTO dbo.LocationTemp2
FROM dbo.LocationTemp1;


-- Split file, line, col of before after info
SELECT id AS rd_id, file_before, file_after, file_location_before, file_location_after
, CASE 
	WHEN has_line_before = 1 
...
INTO dbo.RdPreStagingLocation
FROM dbo.LocationTemp2;


SELECT * FROM dbo.RdPreStagingLocation;
DROP TABLE IF EXISTS dbo.LocationTemp1;
DROP TABLE IF EXISTS dbo.LocationTemp2;
DROP TABLE IF EXISTS dbo.LocationTemp3;
--DROP TABLE IF EXISTS dbo.RdPreStagingLocation;